...
Approach:
1. Use SIFT feature matching to compute a homography that aligns the two images
   into a common epipolar geometry (stereo rectification via homography)
2. Apply OpenCV's StereoSGBM (Semi-Global Block Matching) for dense disparity
3. Filter and smooth the disparity map

The disparity (horizontal pixel offset between matched points) encodes relative
elevation. Larger disparity = higher terrain.

Input: Phase 2 corrected TIFs
...
def align_stereo_pair(aft, fwd):
    ...
    This acts as a simplified stereo rectification — after warping, corresponding
    points should lie on approximately the same horizontal scanline, with remaining
    horizontal offset being the parallax (disparity) that encodes elevation.
    """
    print("  Computing stereo alignment...")
    ...
    # Compute homography to warp fwd onto aft's coordinate system
    ...
    inlier_src = np.float32([kp1[good[i].queryIdx].pt for i in range(len(good)) if mask.ravel()[i]])
    inlier_dst_warped = cv2.perspectiveTransform(
        np.float32([kp2[good[i].trainIdx].pt for i in range(len(good)) if mask.ravel()[i]]).reshape(-1, 1, 2), H
    ).reshape(-1, 2)
...
def compute_disparity(aft, fwd_warped):
    ...
    # SGBM parameters — tuned for satellite stereo imagery
    # The images have large parallax range due to terrain variation
    min_disp = -64
    num_disp = 192  # must be multiple of 16
    block_size = 9
    ...
    # SGBM expects 8-bit images
    disparity_raw = stereo.compute(aft, fwd_warped)

    # disparity_raw is in fixed-point: actual disparity = disparity_raw / 16.0
    disparity = disparity_raw.astype(np.float32) / 16.0

    # Mask invalid disparities (SGBM returns -1 for invalid)
    valid_mask = disparity > (min_disp - 1)
...
def filter_disparity(disparity, valid_mask):
    ...
    # Fill invalid pixels with median of nearby valid pixels
    ...
    # Additional Gaussian blur for a smoother heightmap
...
def main():
    ...
    print("=== Phase 3: Stereo Matching ===\n")

    # Load Phase 2 outputs
    print("Loading corrected images:")
    aft = np.array(Image.open(os.path.join(INPUT_DIR, 'aft_crop_corrected.tif')))
    fwd = np.array(Image.open(os.path.join(INPUT_DIR, 'fwd_crop_corrected.tif')))
    ...
    if valid_mask.sum() > 0:
        ...
        raw_vis[valid_mask] = np.clip((disparity[valid_mask] - v.min()) / (v.max() - v.min()) * 255, 0, 255).astype(np.uint8)
